/**
 * Physics table sandbox — restored rigid-body card table from the pre-netcode era.
 *
 * Grab constraints, ballistic X-drops, Z-take into a camera-space fan, C-marquee
 * group select, S to square a group into one stack body. The impulse solver owns
 * where cards come to rest; nothing snaps.
 */

import * as THREE from 'three'
import { OrbitSpring as Spring } from './orbitSpring'
import { World, BodyMode, TUNING, qSlerp, Body as PhysBody } from '../physics'
import {
  createCardMesh,
  setHighlight,
  Highlight,
...
import { angularVelocityTo, insertionIndexAt, makeRandom, planPlayThrow } from './tableActions'
import { showPrompt, movePrompt, hidePrompt, showMarquee, hideMarquee, setHint } from './tableUi'
...
export type SandboxApi = {
  ...
  onHandHidden: ((hidden: boolean) => void) | null
}

export function createSandbox(container: HTMLElement): SandboxApi {
  let disposed = false
  let handHidden = false
  let onHandHidden: ((hidden: boolean) => void) | null = null
...
const onKeyDown = (e: KeyboardEvent) => {
  ...
  if (k === 'h') {
    ...
    for (const c of cards) {
      if (c.inHand) c.mesh.visible = !handHidden
    }
    // Keep overlay button label in sync if the host wired a listener.
    onHandHidden?.(handHidden)
    return
  }
  if (k === '`') {
    hudOn = !hudOn
...
function animate(): void {
  ...
  for (const card of cards) {
    ...
    if (card.inHand) card.mesh.visible = !handHidden
...
animate()
  ...
  setHint('drag to move  ·  Z take  ·  X drop here  ·  hold C to select, S to square into a stack  ·  H hide hand')
